...
private slots:
    ...
    void testLateAbortHard()
    {
        ...
        auto connection = connect(&fakeFolder.syncEngine(), &SyncEngine::aboutToPropagate, [&](SyncFileItemVector &items) {
            QCOMPARE(items.size(), 1);
            QCOMPARE(items[0]->_file, QLatin1String("A"));
            QCOMPARE(fakeFolder.syncJournal().getFileRecord("A/a0")._etag, fakeFolder.remoteModifier().find("A/a0")->etag.toUtf8());
        });
        QVERIFY(fakeFolder.syncOnce());
